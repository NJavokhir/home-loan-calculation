<template>
  ...
  <v-card class="legalfee-card">
    ...
    <!-- Downpayment input field with unit selection (percentage or RM) as design-->
...
// We are not using downPayment value to calculate Legal Fee
...
const calculateLegalFee = () => {
  ...
  if (price <= 500000) {
    ...
    fee = 9000 + (price - 1000000) * 0.006; // 0.6% for property between RM 1,000,001 and RM 3,000,000
  } else if (price <= 7500000) {
    fee = 21000 + (price - 3000000) * 0.005; // 0.5% for property between RM 3,000,001 and RM 7,500,000
  } else {
    fee = Math.min(46000 + (price - 7500000) * 0.01, price * 0.01); // Negotiable for > RM 7,500,000
  }

  // Apply discount for first-time buyers
  if (isChecked.value) {
    fee *= 0.9; // Example: 10% discount for first-time home buyers
  }
